...
\label{sec:rip}
	...
	\subsubsection{协议流程} % (fold)
		...
		\begin{itemize}
			\item 路由器在进入区域时，向所有邻居发送request报文，邻居则返回一个response报文，里面含有邻居的路由信息。
			\item 该路由器会定时地向所有的邻居发送response报文，即就是发送自己的路由表。
			\item 收到一个response报文意味着要维持该邻居的状态；而如果在一段时间内没有收到某个邻居路由器的response报文，则认为该路由器故障或离开该区域。
			\item 当收到其它邻居路由器的路由信息时，根据该信息更新自己的路由表；当自己的距离矢量更新时，向所有邻居发送自己新的路由表。
		\end{itemize}
